...
def send_email_with_photo(filepath):
    ...
    try:
        ...
        print("Plik odczytany poprawnie.")  # Debugowanie

        # Kodowanie zdjęcia w base64
        try:
            encoded_photo = base64.b64encode(photo_data).decode('utf-8')
            print(f"Zakodowane zdjęcie (pierwsze 100 znaków): {encoded_photo[:100]}")  # Debugowanie
        except Exception as e:
            print(f"Błąd przy kodowaniu zdjęcia do base64: {e}")  # Debugowanie błędu kodowania Base64
            return  # Zatrzymanie funkcji, jeśli wystąpił błąd przy kodowaniu
...
"""
        ...
        # Debugowanie całej wiadomości przed wysłaniem
        print(f"Treść wiadomości e-mail:\n{message[:200]}")  # Debugowanie treści wiadomości (pierwsze 200 znaków)
        ...
        print(f"Błąd wysyłania e-maila: {e}")  # Debugowanie błędu wysyłania
